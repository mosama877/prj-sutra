...
const maol =document.querySelector(".maol")
const laot = document.querySelector(".laot")
const profile = JSON.parse(localStorage.getItem("profile")) ?? []

const usrr = JSON.parse(localStorage.getItem("usrr"))
usrr.forEach(function(e){
    bttn.addEventListener("click", function(klc){
        console.log(e.email == email.value);
        const em = email.value
        const paso = pass.value 
        klc.preventDefault()
        if(email.value == e.email && pass.value == e.password){
            laot.textContent ="تم التسجيل"
            maol.textContent = ""
            const bj = usrr.find(product => product.password == pass.value)
            profile.push({...bj})
            localStorage.setItem("profile", JSON.stringify(profile));
            let lclstrg = JSON.parse(localStorage.getItem("profile"))
            let newdata = lclstrg.filter(function (item) {
                        if (item.password == bj.password) {
                            return item
                        }
            })
        ...
        else if(email.value !== e.email && pass.value == e.password){
            maol.textContent =" الايميل خطا"
            laot.textContent =""
        }
        else if(e.email !== em && e.password !==  paso ){
            maol.textContent =" الايميل و كلمة المرور خطا"
            laot.textContent =""
        }
        else if(email.value == e.email && pass.value !== e.password){
            maol.textContent ="  كلمة المرور خطا"
            laot.textContent =""
        }
